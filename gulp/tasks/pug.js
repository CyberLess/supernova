    ...
    return () => {
	    ...
	    var stream = 
			// -------------------------------------------- Start Task
		        ...
		        .pipe(plugins.replace(/<svg.*?(>)/g, match => {
		            ...
		            let name = match.match(/\bicon-[a-zA-Z]*\b/g).toString().replace('icon-', '')

		            viewport = fs.readFileSync(`src/icons/${name}.svg`, "utf8").match(/viewBox=\"([^']*?)\"/g).toString();
		        ...
		        .pipe(plugins.replace(/(css\/|\/css\/)|(js\/|\/js\/)/g, match => {
		        ...
		        .pipe(plugins.replace(/<img.*?src="(.*?)".*?(>)/g, match => {
